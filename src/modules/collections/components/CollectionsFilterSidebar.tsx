...
export function CollectionsFilterSidebar({
  ...
  const activeFilterCount =
    selectedTypeIds.length + selectedOccasionIds.length + selectedCategoryIds.length;
  const hasActiveFilters = activeFilterCount > 0;

  return (
    <aside className="w-full lg:w-64 shrink-0 lg:sticky lg:top-24 self-start z-30 lg:space-y-8 max-h-none lg:max-h-[calc(100vh-120px)] overflow-visible lg:overflow-y-auto custom-scrollbar pr-0 lg:pr-2 py-1">
      {/* Mobile Toggle Button */}
      <div className="lg:hidden flex items-center justify-between bg-surface-container-low dark:bg-surface-container-high p-4 rounded-sm border border-outline-variant/20 mb-4">
        <button
          type="button"
          onClick={() => setMobileExpanded(!mobileExpanded)}
          className="flex items-center gap-2 font-HelveticaNow text-xs uppercase tracking-widest text-primary dark:text-on-surface font-bold cursor-pointer"
        >
          <span className="material-symbols-outlined text-[18px]">tune</span>
          <span>Filter & Refine {hasActiveFilters && `(${activeFilterCount})`}</span>
      ...
      <div
        ...
        } space-y-8 bg-surface-container-lowest/50 dark:bg-surface-container-low/30 lg:bg-transparent p-4 lg:p-0 rounded-sm border lg:border-none border-outline-variant/15 mb-6 lg:mb-0`}
      >
        {/* Desktop Header & Reset Button */}
        <div className="hidden lg:flex justify-between items-center border-b border-outline-variant/20 pb-4">
